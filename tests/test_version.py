...
def test_version_has_display_and_numeric_forms():
    ...
    assert VERSION_TUPLE == (1, 0, 13, 0)
    text = windows_version_info_text()
    assert "14matsu-ai" in text
    assert "NivisViewer.exe" in text
    assert __version__ in text
    assert "StringStruct('FileVersion', '1.0.13')" in text
    assert "StringStruct('ProductVersion', '1.0.13')" in text
    assert "filevers=(1, 0, 13, 0)" in text
