...
Raster bookのbackground warm-upは、最初のmatching frameがpaintされるまではcurrentだけを処理し、その後はcurrent、進行方向の直近、反対側の直近、距離順の残りというbook-wide orderを1 jobずつ進む。方向優先設定が無効なら数値上のnext側を先にする。新しいnavigationは未開始orderを即時に置換し、実行中の不要なbackground jobはcancel／stale化する。`disabled`以外のprefetch presetでは、旧custom画像件数ではなくcombined byte budgetがRasterの停止位置を決める。custom件数はPDF、RAR／7z、未移行source向けに残す。magnifier、actual size、manual zoom、pixel modeなど原寸pixelを必要とするfull-source要求は例外としてcurrent-onlyであり、近傍の原寸decodeでmemoryを使い切らない。

`viewer_memory_mode`はprefetch件数から独立した共有設定で、`auto`、`minimal`（128 MiB）、256 MiBから32 GiBまでの固定bucketを文字列として保存する。固定modeは対応するbyte値をそのまま使う。`auto`はViewerWindowが最初にそのmodeを読む時または明示的なmode変更時だけ、Windowsのtotal／available physical memory、process working set、現在のcache bytesを1 snapshotで取得する。OS reserveは2 GiBとtotal memoryの20%の大きい方で、残せる容量以下の最大bucketへ切り下げる。既存cacheはavailable memoryへ一度だけ加え戻して二重課金を避けるが、availableがreserveを下回る時はその不足分を既存cacheからも差し引くため、memory pressure下の大きなcacheを固定しない。book切替ごとには再計算せず、available memoryの短期変動でbudgetを揺らさない。snapshot取得不能時は512 MiBへ安全に戻る。

Raster runtimeではpreview／full decoded sourceとdisplay-ready frameを1つのcombined byte budgetへ計上し、旧`ImageCache.cache_size`の小さなpage上限を適用しない。display unit数の上限はbook全体を有限な安全境界として使い、byte admissionとwork-order-aware evictionが実際の保持量を決める。source storeとframe storeは挿入／置換／削除ごとにtotal bytesとlargest artifactを増分更新し、work-order rank mapと遅延構築したeviction orderから各storeのworst候補を連続して取り除く。memory pressureのたびに全artifactを再集計・再sortしない。満杯時は低優先artifactから回収できる容量を非破壊でworkerへ予約し、decode成功、最新request一致、actual source/frame cost確定後にだけQPixmap upload前の入替を行う。cancel／stale／exact admission拒否は既存ready cacheを失わない。予算を増やすとpaint済みfrontierをtimerなしで再開し、容量不足のkeyだけを再試行する。予算を縮小すると実行中のnon-current backgroundをcancelし、worst-firstに一括evictした後、新budgetでwork orderを再driveする。current／complete spread／last-painted frame、ready-hit、atomic commit、paint acknowledgementはこのwarm-upより優先される。PageList thumbnailは別runtimeと小さい専用budgetを持ち、main Viewerのcombined budgetへ二重計上しない。
...
既知の残差として、寸法が遅延判明した横長pageはdecoded sourceを再利用できるが、single／spread／wide-splitのdisplay-unit topologyが変われば対応するdisplay-ready frameを再生成する。非常に大きいbookでは`ViewerWindow._zip_runtime_request`がnavigationごとに全display-unit orderをmaterializeするためO(N)であり、10,000ページ級以上ではready-hitの手前で無視できないcostになり得る。PageList runtimeは生成時にmain budgetの1/4を8～64 MiBへclampした独立budgetを受けるが、開いているPageList runtimeのlive resize APIはまだなく、memory mode変更はmain Viewer cache／Raster runtimeへ即時反映されても既存PageList runtimeへは反映されない。
