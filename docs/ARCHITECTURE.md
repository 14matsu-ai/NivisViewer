...
- `RasterBookRuntime`: FolderとZIPが共有するbook-scoped main Viewer engine。current中心のbook-wideな置換可能work order、decode→transform→layout frame生成、preview／full decoded-source tier、layout依存frame store、実decoded byteとframe byteを合わせたbudget、ready hit bypass、cancel／stale／commit-gated warm-up／paint acknowledgementによる保持境界／callback-drained shutdownを所有する。通常動作はFolder／ZIPとも1 job。JPEG preview要求はdisplay unit、fit mode、physical viewport、DPR、rotation、spread／split slotからworker側で必要pixel数を決める。寸法未索引のbackground unitはworkerでheaderを確認し、unit全体のsource＋frame costをpixel decode前に確定する
- `FolderRasterBookRuntime`: `FolderImageSource`へ共通runtime契約を適用する薄いsource境界。通常のFolder表示と画像pathから開いた親Folder bookのmain表示では、`ImageCache -> ViewerRenderTask -> prepared display`を通らない。JPEG previewはPillowのnative draft縮小を使い、orientation／adjustment／rotationを縮小sourceへ適用する。各file handleはdecode完了時に閉じ、不要な原寸detach copyを作らず、decoded QImageだけをbook lifetimeへ保持する。通常は全window共通のViewer枠1本で1 jobずつ実行する。Folder専用の追加枠は試験用として残すが通常起動では無効。Browser枠は独立する。started compatible wheel workは完了させ、未開始work orderだけ置換する。未表示のページへホイール入力で進んだ後は同方向の追加入力を保留せず消費し、逆方向なら最後の表示済みページへ戻せる。ready frameは同期commitし、描画はQtのupdateで依頼する。worker-finishedのGUI callback待ちも含めて見積りbyteを予約し、容量の二重使用を防ぐ
...
Raster bookのnavigationは`NavigationAdmissionPolicy`でdiscrete、wheel、初回key、key repeat、slider scrub、内部refreshを分離する。discreteは直ちに`request`する。cold wheelは表示単位が未準備の間、同方向の追加notchを消費して論理位置を先へ飛ばさず、逆入力では表示済みの単位へ戻る。受付したwheel requestは入力間隔によらず直ちにruntimeへ渡す。runtimeは未着手要求を最新targetで置換し、同じbook／renderに適合する実行中wheel workを完了させてcacheへ保持する。非wheelの不要な進行中処理やbook／render不適合な処理はcancelする。slider dragも先頭targetと、直前に受付したtargetから50msを超えたpaced targetはcoldでも`request`し、decodeが追いつく間はthumb移動中に表示を更新する。受付済みtargetから50ms以内の連続slider packetは最新targetだけを`stage`し、間隔が閾値を超えた時点でその時の最新targetを周期的に受付するため、10〜16msの連続pointer streamでもpacketごとのdecodeを積み上げない。releaseでは最後のstage targetを確定する。`stage`はkey repeat／sliderで最新current／方向／保持順位を即時反映して旧prefetchを止めるがdecodeを開始しない。key repeatは初回pressを即処理し、repeat中はworkerの瞬間的なidleに左右されずstageしてreleaseで確定する。ready frameは全input kindでtimer／worker／QPixmap再生成なしにcommitし、last-painted frameはreplacement paintまで保護する。decoded sourceがevictされてもlayout済みQPixmapは独立してready hitを維持し、拡大鏡が必要なsourceだけを同じruntimeで再取得する。slider／statusと`ViewerPresentationState`はatomic commit時に同期し、PageList scroll、thumbnail work、history／progress persistenceはmatching paint acknowledgement後へcoalesceする。

Raster bookのbackground warm-upは、最初のmatching frameがcommitされるまではcurrentだけを処理し、その後はcurrent、進行方向の直近、反対側の直近、距離順の残りというbook-wide orderを進む。`RasterBookTopology`はbook／layout revisionごとに一度だけ構築し、各navigationの`RasterWarmupPlan`はcurrent前後をlazyに列挙する。このため10,000ページ級でも入力ごとの全件sort／全件queue再構築を行わない。新しいnavigationは未開始cursorを即時に置換し、実行中の不要なbackground jobはcancel／stale化する。ZIP／Folderでは旧prefetch preset／件数を参照せず、combined byte targetだけが停止位置を決める。custom件数はPDF、RAR／7z、未移行source向けに残す。magnifier選択中／使用中はcurrent-onlyを維持する。安定したactual size、manual zoom、nearestはfull-source fidelityを保ったまま、既存workerのexact admissionとsource＋display＋in-flight予算内でbackground warm-upする。連続zoom中だけ150 ms single-shotでbackgroundを抑制し、current表示は遅延させない。

`viewer_memory_mode`はprefetch件数から独立した共有設定で、`auto`、`minimal`（128 MiB）、256 MiBから32 GiBまでの固定bucketを保存する。Autoの初期bucket選択、OS reserve（2 GiBとtotal physical memoryの20%の大きい方）、固定modeのhard limitは維持する。activeなraster Viewerで実cache利用率がhardの75%以上、pressure ceilingが87.5%以上、未完の容量制約workがある場合だけ、10秒の継続余裕を確認して5秒以上ごとに最大256 MiBずつAuto hard limitを回復させる（上限32 GiB）。欠測、不正sample、時刻逆行／長いsample gap、inactive、需要消失は成長観測をリセットする。active soft targetはhardの87.5%、inactiveは50%。active→inactive後の10秒だけ前者を保持するが、実pressureは即時に優先する。既存5秒sampleで猶予期限を適用し、新pollerは追加しない。Windowは既存BookSession経由で現在と将来のruntime limitsを更新する。Browser brokerはBrowser全体のみを管理し、Viewerとの未使用予算の原子的な共通予約ではない。

Raster runtimeではpreview／full decoded sourceとdisplay-ready frameを1つのcombined byte ledgerへ計上し、旧`ImageCache.cache_size`、source page cap、frame unit capを適用しない。保持可能件数に最大値はなく、実byte costとsoft／hard targetだけが保持量を決める。current／complete spread／last-painted frameと直後・直前の表示単位は最低保証として保護するが、これは最大件数ではない。evictionはlazy work-order rank、方向、layout／DPR／rotation variant、full tier、再生成cost、recencyを共通順位へ写像し、sourceとframeのうち価値が低いものから解放する。満杯時のworker admission budgetは現在の空き容量だけではなく、低優先artifactから安全に回収できる容量も含む。とくにcurrent／navigation sourceでも、独立したdisplay-ready QPixmap frameが残って再表示できるsourceは再decode可能な回収候補になり得る。候補は非破壊で予約し、decode成功、最新request一致、actual source/frame cost確定後に全QPixmap uploadを成功させてから入替を確定する。cancel／stale／failure／upload失敗／exact admission拒否は既存ready cacheを失わない。通常backgroundはsoft targetで停止し、currentと直近の最低保証だけはhard limit側まで低優先artifactと入れ替えられる。1個のoversized background unitはskipして後続の小さいunitを継続し、同じplan内では再試行しない。targetを増やすとpaint済みcursorをtimerなしで再開し、進行中jobは新targetを受け取る。より高優先のcapacity skipが再び収容可能になった場合だけ再優先し、単なる段階回復ではdecodeを捨てない。targetを縮小するとnon-current jobをcancelして遠方からevictする。PageList thumbnailは別runtimeと小さい専用budgetを持ち、main Viewerのcombined ledgerへ二重計上しない。

Raster JPEGの通常fitでは、原寸sourceを常設の正解とせず、完成frameに必要なphysical pixel数へ品質mode別の余裕（`standard` 1.0倍、`smooth` 1.25倍、`moire_reduction`／`high_quality` 2.0倍）を加えたpreviewをdecodeする。90／270度rotationはdecoder座標へ必要寸法を逆写像し、spreadはunit全体ではなく各slotの寸法を使う。previewとfullは同じpageで共存でき、通常fitは最小の十分なpreview、actual-size／manual zoom／magnifier／pixel modeなど原寸pixelの意味が必要な要求だけはfull tierを選ぶ。Folderのprefetch admissionはexact表示targetではなくPillow/libjpegのnative 1/2／1/4／1/8 tierを予算化する。寸法未索引のbackground workはGUI threadでは有界な仮見積りだけを使い、workerがdisplay unit内の未cache pageをすべてheader probeして、保持source rasterとcomplete layout frameの合計をpixel decode前にexact admissionする。これは片軸fit JPEGだけでなくPNG／WebP等の非JPEGにも適用する。budgetを超える巨大keyは現requestのcapacity skipとして記録し、後続の小さいkeyのwarm-upを継続する。新requestまたはbudget増加でcapacity skipだけを再試行し、broken/start failureをloopさせない。full tierの近傍もexact admission内で準備できるが、magnifier中はcurrent-onlyを維持する。magnifierを取り消した時点ではnormal preview requestを同期的にadoptして進行中full jobをstale/cancelし、保持済みframeへ戻る。fullを得ても通常fit用previewと完成QPixmapを直ちに捨てないため、lens終了後は再decodeせずready frameへ戻れる。NivisViewer固有のhigh-DPI target、complete-spread atomic commit、`ViewerPresentationState`、旧frame保持、paint acknowledgementはこのpixel削減より外側の契約として維持する。

既知の残差として、寸法が遅延判明した横長pageはdecoded sourceを再利用できるが、single／spread／wide-splitのdisplay-unit topologyが変われば対応するdisplay-ready frameを再生成する。PageModelの表示単位indexは二状態のpairing変換を合成するsegment treeで、wide判定が変わったpageのleafと親だけを更新するため、後続全体へ見開き位相がずれてもheader batchごとの全件再走査を行わない。Raster runtimeはこのindexをlive O(1)-storage adapterで参照し、batchごとに別の全冊topologyを作らない。Folderの一般的な画像形式は`QImageReader.size()`によるworker上のheader readを使い、QtにないAVIF／JXLだけPillowの既存header経路へ戻る。PageList runtimeは生成時にmain budgetの1/4を8～64 MiBへclampした独立budgetを受けるが、開いているPageList runtimeのlive resize APIはまだなく、memory mode変更はmain Viewer cache／Raster runtimeへ即時反映されても既存PageList runtimeへは反映されない。
...
内部drag-and-dropとWindowsのごみ箱移動は実装済みです。完全削除とZIP／CBZ内部エントリの変更は対象外です。Ctrl+Zはsession内の一段Undoとして既存FileOperationQueueへ再投入し、対応する成功済みcopy／move／rename／新規folder／ZIPだけを記録します。fileはvolume serial、file ID、種別、size、mtimeを再確認し、folderは空の場合だけ逆操作します。link／reparse、replace、folder merge、partial／cancelled operation、identity不一致はUndoを拒否し、再帰削除や上書き復元は行いません。Undo journalはprocess内だけに保持します。
...
可視範囲と選択項目を先に要求します。追加の背景生成は現在の絞り込み済み一覧から、既定で前後各3画面を範囲に含めます。設定で可視範囲のみ、有限画面数、または現在の一覧全体を選べますが、cursorは1 byte／rowだけを保持し、同時に投げる背景要求は1件までです。visible要求があれば先に処理し、BrowserThumbnailProviderの既存メモリ・disk budgetとworker laneを共有します。高速スクロール中は背景生成を止め、停止後に可視範囲を再確認します。絞り込みや一覧更新ではcursorを新しいmodel generationへ結び直し、古いrow結果を流用しません。モデルは正規化パスから行番号への索引を持ち、サムネイル後着時の項目検索を項目数に依存しない処理にします。

Browserの選択表示はファイル名の色不透明度、選択枠の太さ、システムまたは指定色を個別設定します。`Ctrl+F`と`Ctrl+Z`は既存のBrowser shortcut mapへ追加し、以前からのcustom bindingと明示的な未割当を保ちます。検索履歴popupは既存MRUを再利用し、日本語IMEのpreedit中はshortcutを横取りしません。テキスト入力欄のnative UndoはBrowserのファイル操作Undoへ渡しません。
...
Explorerからのclipboard CUTは`Preferred DropEffect`が単一のMOVE値として読める場合だけ
外部MOVEとして扱い、COPY／MOVE混在や未知bitは曖昧な要求として拒否する。paste前の
Viewer確認後とqueue投入直前にclipboard sequence、mime object、URL列、effectを再検証
する。操作完了後のclipboard cleanupも同一snapshotがまだ所有されている場合だけ行い、
貼り付け中に置き換えられた新しいclipboardを消さない。
...
ホイールでは表示準備済み／未表示のどちらのページも、入力間隔による待ち時間を置かず`RasterBookRuntime.request()`へ渡す。ready frameは同期commit後に描画も促し、再中心化した先読み順で空いている実行枠を使う。未表示のcurrentは背景処理より優先するが、同じ本・同じ描画条件で既に始まった処理は完成させてcacheへ収める。未着手の仕事だけを最新targetで置き換え、実行枠が空いたら最新currentを先に始める。画面への公開は最新要求IDだけに限定する。book切替・shutdown・非互換の描画条件・メモリ圧力では従来のcancelを維持し、discrete/key/sliderのcold操作も従来どおり不要な進行中処理をcancelする。これによりホイールでの有用なdecode結果を捨てず、実行枠と要求数を増殖させない。完成待ちの1処理分だけ最終targetが遅れる可能性がある。世代番号と要求IDによる古い結果の排除、方向反転、最終ページ確定の契約を維持する。
